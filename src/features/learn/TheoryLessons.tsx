  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <ul
        ...
        className="border-border flex shrink-0 gap-px overflow-x-auto border-b"
      >
        {LESSONS.map((lesson) => (
          <li key={lesson.id}>
            <button
              ...
              className={`px-3 py-1.5 text-left font-mono text-[11px] whitespace-nowrap ${
                lesson.id === lessonId
        ...
        <div className="min-h-0 grow overflow-y-auto p-3">
          ...
          <ul className="mt-2 flex flex-col gap-1">
            {notes.points.map((point) => (
              <li key={point} className="text-text text-sm">
                {point}
              </li>
            ))}
          </ul>
          ...
          <div className="mt-4 flex items-baseline justify-between">
            <h3 className="text-text-muted font-mono text-[11px] tracking-widest uppercase">
              Compruébalo
            </h3>
            ...
            {notes.exercises.map((exercise, index) => {
              ...
              return (
                <li key={exercise.prompt}>
                  <fieldset>
                    <legend className="text-text text-sm">{exercise.prompt}</legend>
